...
class LabMigrationUploadCodeForm extends FormBase {
  ...
  //alert('ok');
    ...
    $form['sourcefile']['samplemarkup'] = [
      '#type' => 'item',
      '#markup' => "<a style='float:right;padding-right:100px' href='http://esim.fossee.in/resource/book/analysis_of_BJT_amplr.pdf' target='_blank'> View Sample PDF</a><label style='float:right;padding-right:20px'>For PDF reference :</label> ",
    ];
    $form['sourcefile']['chppdf'] = [
      '#type' => 'file',
      '#title' => t('Upload PDF File'),
      '#size' => 48,
      '#description' => t('No spaces or any special characters allowed in filename.') . '<br />' . t('Allowed file extensions : ') . variable_get('lab_migration_pdf_extensions', ''),
    ];
...
*/
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if (!$form_state->getValue(['esim_version'])) {
      $form_state->setErrorByName('esim_version', t('Please select the esim version used.'));
    }
    ...
    if (isset($_FILES['files'])) {
      ...
      /* check if atleast one pdf file is uploaded */
      if (!($_FILES['files']['name']['chppdf'])) {
        $form_state->setErrorByName('chppdf', t('Please upload pdf file.'));
      }
        ...
        if ($file_name) {
          ...
          switch ($file_type) {
            case 'S':
              ...
              break;
            case 'P':
              $allowed_extensions_str = variable_get('lab_migration_pdf_extensions', '');
              break;
            case 'R':
    ...
    foreach ($_FILES['files']['name'] as $file_form_name => $file_name) {
      if ($file_name) {
        ...
        if (move_uploaded_file($_FILES['files']['tmp_name'][$file_form_name], $root_path . $dest_path . $_FILES['files']['name'][$file_form_name])) {
          if ($file_form_name != "chppdf") {
            /* for uploaded files making an entry in the database */
            $query = "INSERT INTO {lab_migration_solution_files} (solution_id, filename, filepath,pdfpath, filemime, filesize, filetype, timestamp)
          VALUES (:solution_id, :filename, :filepath,:pdfpath, :filemime, :filesize, :filetype, :timestamp)";
            $args = [
              ":solution_id" => $solution_id,
              ":filename" => $_FILES['files']['name'][$file_form_name],
            ...
            $query = "UPDATE {lab_migration_solution_files} SET pdfpath = :pdfpath WHERE solution_id = :solution_id";
            $args = [
              ":pdfpath" => $dest_path . $_FILES['files']['name'][$file_form_name],
              ":solution_id" => $solution_id,
            ];
