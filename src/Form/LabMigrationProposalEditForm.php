...
class LabMigrationProposalEditForm extends FormBase {
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($form_state->getValue(['open_solution']) == 1) {
      ...
      $result = \Drupal::database()->update('lab_migration_proposal')
        ->fields([
        'solution_provider_uid' => 0,
        'solution_status' => 0,
    ...
    //    ":id" => $proposal_id,
    //  );
    $query = \Drupal::database()->update('lab_migration_proposal')
      ->fields([
      'name_title' => $v['name_title'],
      'name' => $v['name'],
      'contact_ph' => $v['contact_ph'],
    ...
    for ($counter = 1; $counter <= 15; $counter++) {
      ...
      if ($experiment_data) {
        ...
        if (strlen(trim($form_state->getValue([$experiment_field_name]))) >= 1) {
          ...
          if (!$result2) {
            drupal_set_message(t('Could not update Title of the Experiment : ') . trim($form_state->getValue([$experiment_field_name])), 'error');
          }
        }
        else {
    ...
    for ($counter = 1; $counter <= 15; $counter++) {
      ...
      if (strlen(trim($form_state->getValue([$lab_experiment_insert]))) >= 1) {
        ...
        if (!$result4) {
          drupal_set_message(t('Could not insert Title of the Experiment : ') . trim($form_state->getValue([$lab_experiment_insert])), 'error');
        }
        else {
          $number++;
        }
